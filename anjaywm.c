...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "anjayWM", 7);
...
wintomon(Window w)
{
	...
	for (m = mons; m; m = m->next)
		if (w == m->barwin || w == m->traywin || w == m->extrabarwin)
			return m;
	if ((c = wintoclient(w)))
		return c->mon;
	return selmon;
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "anjaywm: fatal error: request code=%d, error code=%d\n",
...
{
	die("anjaywm: another window manager is already running");
	return -1;
}
...
main(int argc, char *argv[])
{
	...
	if (!(dpy = XOpenDisplay(NULL)))
		die("anjaywm: cannot open display");
	checkotherwm();
	XrmInitialize();
	load_xresources();
	setup();
